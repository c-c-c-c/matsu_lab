...
#  畳み込み演算（空間フィルタリング）
def ﬁlter2d(src, kernel, ﬁll_value=-1):
    #  カーネルサイズ
    m, n = kernel.shape
...
# Non maximum Suppression 処理
def non_max_sup(G, Gth):
    h, w = G.shape
    dst = G.copy()
    #  勾配方向を 4 方向 ( 垂直・水平・斜め右上・斜め左上 ) に近似
    ...
    #  注目画素と勾配方向に隣接する 2 つの画素値を比較し、注目画素値が最大でなければ 0 に
...
def canny_edge_detecter(gray, t_min, t_max, d):
    ...
    kernel_g = np.array([[1/16, 1/8, 1/16],
                         ...
                         [1/16, 1/8, 1/16]])
    #  「ガウシアン」フィルタ
    G = ﬁlter2d(gray, kernel_g, -1)
    
    #  処理 2  微分画像の作成（ Sobel フィルタ）
    ...
    Gx = ﬁlter2d(G, kernel_sx, 0)
    Gy = ﬁlter2d(G, kernel_sy, 0)
    
    #  処理 3  勾配強度・方向を算出
    G = np.sqrt(Gx**2 + Gy**2)
    Gth = np.arctan2(Gy, Gx) * 180 / np.pi
    
    #  処理 4 Non maximum Suppression 処理
    G = non_max_sup(G, Gth)
    
    #  処理 5 Hysteresis Threshold 処理
